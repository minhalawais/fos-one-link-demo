...
export const SceneOmnichannel = ({ isActive }: { isActive: boolean }) => {
  const [visibleCount, setVisibleCount] = useState(0)
  const [isMerged, setIsMerged] = useState(false)

  // Timing optimized for the Voiceover
  useEffect(() => {
    if (isActive) {
      setVisibleCount(0)
      setIsMerged(false)
      ...
      const sequence = [
        { time: 3500, action: () => setVisibleCount(1) }, // "Multiple channels..."
        { time: 4500, action: () => setVisibleCount(2) }, // "Toll-free..."
        { time: 5500, action: () => setVisibleCount(3) }, // "SMS..."
        { time: 6500, action: () => setVisibleCount(4) }, // "Email..."
        { time: 7500, action: () => setVisibleCount(5) }, // "Mobile App..."
        { time: 10500, action: () => setIsMerged(true) }, // "These options ensure..." -> Merge
      ]

      const timers = sequence.map(item => setTimeout(item.action, item.time))
      return () => timers.forEach(clearTimeout)
    }
  }, [isActive])

  return (
    <div className="w-full h-full flex flex-col items-center justify-center bg-[#F5F5F7] relative overflow-hidden p-4">
      
      {/* Dynamic Background */}
      <div className="absolute inset-0 z-0">
        <div className="absolute inset-0 bg-gradient-to-br from-[#F5F5F7] to-[#E5E5EA] opacity-90 z-10" />
        {/* Subtle animated orbs for modern feel */}
        <motion.div 
      ...
      <div className="relative z-10 w-full h-full flex flex-col items-center justify-center max-w-5xl">
        ...
        >
          <motion.h2 
            ...
            className="text-2xl md:text-3xl lg:text-4xl font-bold text-[#284952] mb-2 tracking-tight"
          >
          ...
          <motion.p 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="text-[#767676] text-sm md:text-base font-medium"
          >
        ...
        <motion.div
           className="h-16 flex-shrink-0 flex items-end justify-center pb-2"
        >
          {isMerged && (
            <motion.div
              ...
              className="flex items-center gap-3 bg-[#284952] text-white px-5 py-2.5 rounded-full shadow-2xl scale-90 md:scale-100"
            >
              <div className="bg-[#60BA81] rounded-full p-1">
                <Users size={16} className="text-white" />
              </div>
              <div className="flex flex-col leading-none">
                <span className="font-bold text-sm">100% Coverage</span>
                <span className="text-[10px] text-white/70">Safe & Anonymous</span>
              </div>
            </motion.div>
          )}
        </motion.div>
