...
const ASSETS = {
  ...
  officer: "/assets/avatars/male_io.png",
}
...
export const SceneNotification = ({ isActive }: { isActive: boolean }) => {
  ...
  useEffect(() => {
    if (isActive) {
      ...
      // Timings based on script:
      // 82s: Scene starts (Stage 0)
      // 87.2s: Mobile/Email appear (5.2s delay)
      const timers = [
        setTimeout(() => setStage(1), 5200),   // Mobile & Email Appears (5.2s)
        setTimeout(() => {
          setStage(2)  // Focus Email
          setEmailScrolling(true)
        }, 6500),  // Give a moment after appearance before focusing
        setTimeout(() => setStage(3), 9500),  // Focus Portal
        setTimeout(() => setStage(4), 10500),  // New Row Entry
        setTimeout(() => setStage(5), 13880), // Cursor Move
      ]
  ...
  return (
    <div className="w-full h-full flex items-center justify-center bg-[#F5F5F7] p-2 md:p-6 relative overflow-hidden font-sans">
      ...
      <div className="relative z-10 w-full max-w-6xl h-full flex items-center gap-8">

        {/* LEFT SIDE: OFFICER CHARACTER */}
        <AnimatePresence>
          {stage < 3 && (
            <motion.div
              ...
              className="hidden md:flex flex-col items-center justify-end h-full w-1/3 relative"
            >
              <motion.img
                src={ASSETS.officer}
                alt="FOS Officer"
              ...
              {/* Notification Wave Effect from Officer */}
              {stage >= 1 && (
                <motion.div
                  initial={{ scale: 0.5, opacity: 0 }}
                  animate={{ scale: 1.5, opacity: 0 }}
                  transition={{ duration: 1.5, repeat: Infinity }}
                  className="absolute top-1/3 right-10 w-20 h-20 border-2 border-[#60BA81] rounded-full"
                />
              )}
            </motion.div>
          )}
